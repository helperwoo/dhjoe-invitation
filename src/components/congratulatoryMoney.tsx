...
const CongratulatoryMoney = () => {
  ...
  return (
    <Wrapper>
      ...
      >
        <div>
          <b>부 : {config.groom.f_name}</b>
          <Divider type="vertical" />
          <CopyToClipboard text={config.groom.f_account}>
            <Button
              type="text"
              style={{ padding: 0, margin: 0 }}
              onClick={() => message.success("계좌번호가 복사되었습니다.")}
            >
              {config.groom.f_account}
            </Button>
          </CopyToClipboard>
        </div>
        <div style={{ marginTop: 24, marginBottom: 24 }}>
          <b>모 : {config.groom.m_name}</b>
          <Divider type="vertical" />
      ...
      >
        <div>
          <b>부 : {config.bride.f_name}</b>
          <Divider type="vertical" />
          <CopyToClipboard text={config.bride.f_account}>
            <Button
              type="text"
              style={{ padding: 0, margin: 0 }}
              onClick={() => message.success("계좌번호가 복사되었습니다.")}
            >
              {config.bride.f_account}
            </Button>
          </CopyToClipboard>
        </div>
        <div style={{ marginTop: 24, marginBottom: 24 }}>
          <b>모 :{config.bride.m_name}</b>
          <Divider type="vertical" />
